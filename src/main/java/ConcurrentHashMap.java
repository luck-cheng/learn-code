 ...
 *
 *
 * 由Doug Lea在JCP JSR-166专家组成员的协助下撰写并发布到公共领域，如下所述
 ...
 *
 * 一个支持完全并发检索和高预期并发性更新的哈希表。该类遵循与{@link java.util.Hashtable}相同的功能规范，
 * 并包含与{@code Hashtable}的每个方法相对应的方法版本。,但是，即使所有操作都是线程安全的，
 * 检索操作也不需要锁定，并且不会以阻止所有访问的方式锁定整个表。,在依赖于线程安全但不依赖于其同步细节的程序中，
 * 此类可与{@code Hashtable}完全互操作。
 *
